...
// ===== 3D関節座標（正規化: x=左右, y=上下 0-1, z=前後 正=前方） =====
// 正しいスクワットフォーム: 膝はつま先方向に追従, がに股なし
const JOINTS_STAND = {
  head:      [ 0.00, 0.07,  0.00],
  neck:      [ 0.00, 0.14,  0.00],
  shoulderL: [-0.13, 0.21,  0.00],
  shoulderR: [ 0.13, 0.21,  0.00],
  elbowL:    [-0.16, 0.33,  0.02],
  elbowR:    [ 0.16, 0.33,  0.02],
  wristL:    [-0.14, 0.44,  0.02],
  wristR:    [ 0.14, 0.44,  0.02],
  hipL:      [-0.09, 0.47,  0.00],
  hipR:      [ 0.09, 0.47,  0.00],
  kneeL:     [-0.09, 0.68,  0.00],  // 膝: 足首の真上（がに股なし）
  kneeR:     [ 0.09, 0.68,  0.00],
  ankleL:    [-0.09, 0.88,  0.00],
  ankleR:    [ 0.09, 0.88,  0.00],
  toeL:      [-0.11, 0.92,  0.10],  // つま先: やや前方・外向き
  toeR:      [ 0.11, 0.92,  0.10],
}

// 正しいスクワット姿勢（パラレルスクワット）
const JOINTS_SQUAT = {
  head:      [ 0.00, 0.25,  0.06],  // やや前傾
  neck:      [ 0.00, 0.32,  0.03],
  shoulderL: [-0.14, 0.38,  0.01],
  shoulderR: [ 0.14, 0.38,  0.01],
  elbowL:    [-0.20, 0.46,  0.13],  // 腕を前に: バランス維持
  elbowR:    [ 0.20, 0.46,  0.13],
  wristL:    [-0.22, 0.52,  0.21],
  wristR:    [ 0.22, 0.52,  0.21],
  hipL:      [-0.11, 0.60,  0.02],
  hipR:      [ 0.11, 0.60,  0.02],
  kneeL:     [-0.12, 0.73,  0.18],  // 膝: 足首上方・前方（つま先追従, 広がりすぎない）
  kneeR:     [ 0.12, 0.73,  0.18],
  ankleL:    [-0.10, 0.88,  0.00],
  ankleR:    [ 0.10, 0.88,  0.00],
  toeL:      [-0.13, 0.92,  0.12],
  toeR:      [ 0.13, 0.92,  0.12],
}
...
const VIEWS = {
  front:    ([x, y])       => [x, y],                      // 正面: XY平面
  side:     ([,  y, z])    => [z * 1.5, y],                // 真横: ZY平面（z方向を拡大）
  diagonal: ([x, y, z])    => [x * C30 + z * S30, y],      // 斜め30°
}
